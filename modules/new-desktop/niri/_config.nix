{ lib, config, theme }:
let
  inherit (config.desktop.niri) outputs;

  renderOutput = name: cfg: ''
    output "${name}" {
        mode "${cfg.mode}"
        scale ${toString cfg.scale}
        ${lib.optionalString (cfg ? transform) ''transform "${cfg.transform}"''}
        position ${toString cfg.position.x} ${toString cfg.position.y}
    }
  '';

  decorations = import ./_decorations.nix { inherit theme; };
in
/* kdl */ ''
  input {
      keyboard {
  ...
  ${lib.concatStringsSep "\n" (lib.mapAttrsToList renderOutput outputs)}
  ...
  binds {
      ...
      "Mod+S" allow-when-locked=true { spawn-sh "grim -g \"$(slurp)\" - | wl-copy"; }
      "Mod+Ctrl+S" { spawn-sh "grim -g $(slurp)"; }
      "Mod+Shift+S" allow-when-locked=true { spawn-sh "grim - | wl-copy"; }
      ...
      "Mod+Alt+V" repeat=false { spawn-sh "wl-mirror $(niri msg --json focused-output | jq -r .name)"; }
      "Mod+B" { spawn-sh "if pgrep -x wlsunset >/dev/null; then pkill wlsunset; else wlsunset -t 3500 -S 00:00 -s 00:00 & fi"; }
