...
in
{
  options.hostInstall = lib.mkOption {
    type = lib.types.attrsOf (lib.types.submodule {
      ...
      options.swapSize   = lib.mkOption { type = lib.types.str; default = "32G"; };
      ...
      text = ''
        ...
        while [[ $# -gt 0 ]]; do
          case "$1" in
            ...
            --help|-h)
              ...
              echo "Hosts: ${builtins.toString (builtins.attrNames hostConfigs)}"
              exit 0 ;;
            *) ARGS+=("$1"); shift ;;
          esac
        done
        ...
        case "$HOST" in
          ...
          *)
            ...
            echo "Known: ${builtins.toString (builtins.attrNames hostConfigs)}"
            exit 1
            ;;
        esac
